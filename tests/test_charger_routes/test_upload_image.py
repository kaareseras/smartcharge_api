...
def test_file_upload(mocker_open_image, client, charger, state_status, state_power, test_session, user):
    ...
    with open("tests/test_charger_router/test_images/easee_png.png","rb") as image:
        file = image.read()
...
def test_file_upload_to_invalid_charger(mocker_open_image, client, test_session, user, charger):
    data = _generate_tokens(user, test_session)
    headers = {
        "Authorization": f"Bearer {data['access_token']}"
    }
...
def test_file_upload_bigger_than_1MB(client, test_session, user, charger):
    ...
    with open("tests/test_charger_router/test_images/large.png","rb") as image:
        file = image.read()
...
def test_file_upload_is_not_alowed(client, test_session, user, charger):
    ...
    with open("tests/test_charger_router/test_images/pdf.pdf","rb") as image:
        file = image.read()
