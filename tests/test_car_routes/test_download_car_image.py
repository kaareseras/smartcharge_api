"""
1. User can download the picture for a valid car
2. User cannot download a picture for an invalid car
3. User has to be logged in to download a picture
...
def test_user_can_download_picture_for_valid_car(mock_path, mocker_open_image, client, car, user ,test_session):
...
def test_user_cannot_download_picture_for_invalid_car( client, car, user ,test_session):
...
def test_default_picture_returned_if_no_picture_defined_in_db( client, car, user ,test_session):
    ...
    assert response.status_code == 200
    assert response.headers["content-length"] == '76024'

def test_default_picture_returned_if_picture_not_found_in_file_system(client, car, user ,test_session):
